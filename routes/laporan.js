...
const { getAllPemesananByPeriode, getQtyKategori, getTotalPendapatan } = require("../controllers/laporan");
...
laporan.route("/pendapatan/:periodeAwal/:periodeAkhir").get(async (req, res) => {
    try {
        const {periodeAwal, periodeAkhir} = req.params
        const result = await getTotalPendapatan(periodeAwal, periodeAkhir);
        response.success(result, "mengambil seluruh data laporan untuk Total Pendapatan", res)
    } catch(err) {
        response.error({ error: err.message }, req.originalUrl, 403, res)
    }
});
